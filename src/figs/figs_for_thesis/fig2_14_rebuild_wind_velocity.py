...
def plot_window_with_out_of_range_highlight(window_vel, out_of_range_mask, sensor_id, 
    ...
    # 计算修补后的曲线：对越界点，取左5个+右5个非离群点（不包含离群点本身和其他离群点，自然扩展）
    repaired_vel = window_vel.copy()
    for idx in np.where(out_of_range_mask)[0]:
        ...
        left_count = 0
        left_idx = idx - 1
        while left_count < 5 and left_idx >= 0:
            if not out_of_range_mask[left_idx]:
                neighbors.append(window_vel[left_idx])
                left_count += 1
            left_idx -= 1
        ...
        right_count = 0
        right_idx = idx + 1
        while right_count < 5 and right_idx < len(window_vel):
            if not out_of_range_mask[right_idx]:
                neighbors.append(window_vel[right_idx])
                right_count += 1
            right_idx += 1
        
        # 如果收集到点，计算平均值
        if len(neighbors) > 0:
            repaired_vel[idx] = np.mean(neighbors)
    ...
    if title is None:
        title = f"{sensor_id} - 时间窗口风速分析 (离群点: {out_count}/{len(window_vel)})"
...
def process_sensor_data(sensor_id, wind_metadata, min_vel=0, max_vel=70, max_windows=10):
    """
    ...
    for file_idx, file_info in enumerate(sensor_files):
        # 如果已经收集到足够的窗口，停止筛选
        if len(window_figures) >= max_windows:
            break
    ...
    print(f"\n✓ 数据统计完成")
    print(f"  收集有越界数据的窗口数: {len(window_figures)}")
...
def main():
    ...
    for sensor_id in sensor_ids:
        window_figures = process_sensor_data(sensor_id, wind_metadata, MIN_VEL, MAX_VEL, max_windows=MAX_FIGS)
    ...
    for i, (fig, count, window_info) in enumerate(all_window_figs):
        if i < MAX_FIGS:
            ...
            saved_figs_count += 1
            print(f"  ✓ 保存图表 {i+1}/{min(MAX_FIGS, len(all_window_figs))} - {window_info['sensor_id']} 窗口")
        else:
            plt.close(fig)
    ...
    print(" "*20 + "绘图处理完成")
    print("="*80)
    print(f"✓ 已保存图表数: {saved_figs_count}")
