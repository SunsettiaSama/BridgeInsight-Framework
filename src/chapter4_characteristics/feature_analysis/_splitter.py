...
def save_class_results(
    ...
    for cls_id, samples in sorted(by_class.items()):
        ...
        file_meta = {
            ...
            "source_result":       Path(source_result_path).name if source_result_path else "",
        ...
        if split_by_sensor:
            ...
            for sensor_id, sensor_samples in sorted(by_sensor.items()):
                ...
                tmp_path = sensor_path.with_name(f"{sensor_path.name}.tmp")
                with open(tmp_path, "w", encoding="utf-8") as f:
                    json.dump(payload, f, ensure_ascii=False)
                tmp_path.replace(sensor_path)
            ...
            tmp_path = cls_path.with_name(f"{cls_path.name}.tmp")
            with open(tmp_path, "w", encoding="utf-8") as f:
                json.dump(payload, f, ensure_ascii=False)
            tmp_path.replace(cls_path)
