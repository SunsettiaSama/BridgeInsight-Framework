...
使用方式
--------
    python -m src.chapter4_characteristics.feature_analysis.run \
...
from src.chapter4_characteristics.feature_analysis._modal import (
    compute_psd_top_modes,
    compute_spectral_features,
)
from src.chapter4_characteristics.feature_analysis._signal import compute_time_stats
from src.chapter4_characteristics.feature_analysis._coupling import compute_cross_coupling
from src.chapter4_characteristics.feature_analysis._wind import (
    get_wind_stats_for_sample,
    load_or_compute_wind_stats_by_timestamp,
    compute_reduced_velocity,
)
from src.chapter4_characteristics.feature_analysis._splitter import save_class_results
...
def run(
    ...
    if cfg.enable_wind_stats:
        ...
        wind_cache_path = out_dir.parent / "wind_stats_by_timestamp_cache.json"
        wind_stats_by_ts = load_or_compute_wind_stats_by_timestamp(
            wind_metadata_path=wind_metadata_path,
            cache_path=str(wind_cache_path),
        )
    else:
    ...
    def _batch_done(batch_idx: int) -> bool:
        suffix = f"_batch_{batch_idx:05d}.json"
        return any(out_dir.rglob(f"*{suffix}"))

    if cfg.n_workers <= 0:
        _init_worker(cfg)
        for batch_idx, batch in enumerate(batches, start=1):
            if _batch_done(batch_idx):
                processed += len(batch)
                logger.info(f"跳过已完成 batch {batch_idx}：累计 {processed}/{len(worker_args)}")
                continue
