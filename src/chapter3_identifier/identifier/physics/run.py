...
from src.identifier.physics.base_mode_calculator import Cal_Mount, parse_mount_point_id
...
def _process_file_group_worker(
    args: Tuple[str, List[Tuple[int, int, Dict]], Dict[str, float], dict, dict, int, int, bool]
) -> Dict[int, int]:
    """
    单文件组 MECC 分类 Worker。
    ...
    Parameters
    ----------
    args : (file_path, windows, f0_cache, mecc_params, welch_params, window_size, fs, viv_only)
        windows : [(orig_idx, window_idx, meta_dict), ...]
            meta_dict 仅保留 sensor_id，避免 _SampleRecord 跨进程传输
    """
    file_path, windows, f0_cache, mecc_params, welch_params, window_size, fs, viv_only = args
...
def _build_f0_cache(samples) -> Dict[str, float]:
    ...
    for sid in sorted(sensor_ids):
        ...
        logger.info(f"  {sid} → 测点 {mount_point} → f0 = {cache[sid]:.4f} Hz")
    return cache
...
def _run_mecc_parallel(
    ...
    task_args = [
        (fp, windows, f0_cache, mecc_params, welch_params, window_size, fs, viv_only)
...
def main(config_path: Optional[Path] = None):
    ...
    welch_params  = {k: int(v) for k, v in cfg.get("welch", {}).items()}
    ...
    inplane_predictions = _run_mecc_parallel(
        ...
        f0_cache, mecc_params, welch_params, window_size, fs, num_workers, viv_only,
        "MECC 面内识别",
    )
    ...
    model_info = (
        ...
        f"welch nfft={welch_params.get('nfft', 2048)}, "
        f"nperseg={welch_params.get('nperseg', 1024)}, "
        f"noverlap={welch_params.get('noverlap', 512)}"
    )
