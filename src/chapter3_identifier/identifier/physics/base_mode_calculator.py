...
# ST-VIC-C18-101-01 → C18-101
_MOUNT_POINT_PATTERN = re.compile(r"^ST-VIC-(C\d+-\d+)-\d+$")
...
def parse_mount_point_id(sensor_id: str) -> Optional[str]:
    """从传感器 ID 解析测点编号，如 ST-VIC-C18-101-01 → C18-101。"""
    m = _MOUNT_POINT_PATTERN.match(str(sensor_id).strip())
    if m:
        return m.group(1)
    return None


def _params_from_mount_point(mount_point_id: str) -> Optional[dict]:
    cfg = _load_cable_config()
    cables = cfg.get("cables", {})
    return cables.get(mount_point_id)


def _mount_point_from_sensor(sensor_id: str) -> Optional[str]:
    return parse_mount_point_id(sensor_id)
...
class Cal_Mount():
    ...
    按测点编号查询（需 config/cables/cable_params.yaml 中有对应条目）：
        mount = Cal_Mount.from_mount_point("C34-101")

    按传感器 ID 查询（自动解析测点编号 Cxx-yyy）：
    ...
    @classmethod
    def from_mount_point(cls, mount_point_id: str) -> "Cal_Mount":
        raw = _params_from_mount_point(mount_point_id)
        if raw is None:
            warnings.warn(
                f"[Cal_Mount] 配置文件中未找到测点 '{mount_point_id}'，回退到默认参数。",
                stacklevel=2,
            )
        ...
        if missing:
            warnings.warn(
                f"[Cal_Mount] 测点 '{mount_point_id}' 以下参数尚未填写：{missing}，回退到默认参数。",
                stacklevel=2,
            )
            return cls(**_DEFAULT_PARAMS)
    ...
    def from_sensor(cls, sensor_id: str) -> "Cal_Mount":
        mount_point_id = _mount_point_from_sensor(sensor_id)
        if mount_point_id is None:
            warnings.warn(
                f"[Cal_Mount] 无法从传感器 '{sensor_id}' 解析测点编号，回退到默认参数。",
                stacklevel=2,
            )
            return cls(**_DEFAULT_PARAMS)
        return cls.from_mount_point(mount_point_id)
