...
def build_jobs_router(deps: AppDeps) -> APIRouter:
    ...
    def start_build_cache(round_idx: int = 1):
        _ensure_idle(deps)
        return deps.jobs.start_build_cache(round_idx=round_idx, config_path=deps.config_path)

    @router.post("/api/jobs/train")
    def start_train(round_idx: int = 1):
        _ensure_idle(deps)
        return deps.jobs.start_train(round_idx=round_idx, config_path=deps.config_path)
    ...
    def job_log(round_idx: int = 1, phase: str = "train", tail: int = 8000):
        return {"round_idx": round_idx, "phase": phase, "text": deps.jobs.read_log_tail(round_idx, phase=phase, max_chars=tail)}

    @router.get("/api/jobs/monitor")
    def training_monitor(round_idx: int = 1):
        return build_training_monitor_payload(deps.cfg, round_idx, deps.jobs.poll(), deps.jobs.read_log_tail(round_idx, "train"))

    @router.get("/api/jobs/infer/monitor")
    def infer_monitor(round_idx: int = 1):
        return build_infer_monitor_payload(deps.cfg, round_idx, deps.jobs.poll(), deps.jobs.read_log_tail(round_idx, "infer"))
