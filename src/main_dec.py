...
import datetime
from .proposser.process_data import DataManager
...
class PlotLib():
    ...
    def rose_hist(self, x, 
        ...
        # 如果提供了 wind_velocities，则计算每个 bin 的平均风速
        ...
        max_n = max(n)
        if max_n == 0:
            y_max = 1.0  # 防止无数据时 Y 轴显示异常
        else:
            y_max = max_n * 1.1  # 加 10% 的余量
        ax.set_ylim(0, y_max)
...
def Wind_Rose_Map(wind_dir = r'F:\Research\Vibration Characteristics In Cable Vibration\data\2024September\SuTong\UAN\\'):

    manager = DataManager()
    wind_velocities, wind_directions, wind_angles = manager.get_wind_data_from_root(wind_dir)
    ploter = PlotLib()
    fig, ax = ploter.rose_hist(wind_directions, 
                               separate_bins = 30, 
                               add_fig = True, 
                               density = False, 
                               geographic_orientation = True)
...
def Wind_Rose_Map_Only():
    ...
    unpacker = UNPACK()
    path = r'F:\Research\Vibration Characteristics In Cable Vibration\data\2024September\SuTong\VIC\\'
...
def Wind_Rose_Map_With_VIV():
    ...
    VIV_excel = r'F:\Research\My_Thesis\Vibration Characteristics In Cable Vibration\Img\VIV.xlsx'
