...
def test_naive_bayes_workflow():
    ...
    if 'train' in results and results['train']:
        ...
        print(f"  - 特征维度: {train_result['feature_dim']}")
        print(f"  - 类别数: {train_result['class_num']}")
    
    if 'eval' in results and results['eval']:
        eval_result = results['eval']
        print(f"✓ 推理完成")
        print(f"  - 推理样本数: {eval_result['sample_num']}")
        print(f"  - 类别数: {eval_result['class_num']}")
