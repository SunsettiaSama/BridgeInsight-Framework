...
def run_loop(max_rounds: int = 10, config_path: str | None = None) -> None:
    ...
    for round_idx in range(1, max_rounds + 1):
        ...
        write_job_state(job_path, {"status": "running", "phase": "train", "round": round_idx})
        run_training(round_idx=round_idx, gold_only=None, config_path=config_path)
        write_job_state(job_path, {"status": "done", "phase": "train", "round": round_idx})

        logger.info(f"=== Round {round_idx}: 全量识别 ===")
        write_job_state(job_path, {"status": "running", "phase": "infer", "round": round_idx})
        run_inference(round_idx=round_idx, config_path=config_path)
        write_job_state(job_path, {"status": "idle", "phase": "infer", "round": round_idx})
