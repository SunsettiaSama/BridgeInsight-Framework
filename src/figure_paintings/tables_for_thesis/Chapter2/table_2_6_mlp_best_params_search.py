...
# 读取搜索结果
result_file = str(Path(__file__).parent.parent.parent.parent) + '/results/training_result/deep_learning_module/search_best_hyperparams/mlp_search_result.json'
...
# 提取每个参数组合的最佳F1值
results_with_f1 = []
for result in data['search_results'][:TOPK]:
...
for rank, result in enumerate(sorted_by_f1, 1):
    ...
    print(f'   batch_size: {result["params"]["batch_size"]}, lr: {result["params"]["learning_rate"]}, '
          f'weight_decay: {result["params"]["weight_decay"]}, grad_clip: {result["params"]["gradient_clip_norm"]}, label_smoothing: {result["params"]["label_smoothing"]}')
    print()

# # 按accuracy排序（原始排序方式）
# sorted_by_acc = sorted(results_with_f1, key=lambda x: x['best_val_accuracy'], reverse=True)

# print('=' * 100)
# print('【按Accuracy排序 - 所有参数组合结果详情】')
# print('=' * 100)
# for rank, result in enumerate(sorted_by_acc, 1):
#     print(f'{rank}. Acc={result["best_val_accuracy"]:.6f}, F1={result["best_val_f1"]:.6f}, '
#           f'Precision={result["best_val_precision"]:.6f}, Recall={result["best_val_recall"]:.6f}')
#     print(f'   batch_size: {result["params"]["batch_size"]}, lr: {result["params"]["learning_rate"]}, '
