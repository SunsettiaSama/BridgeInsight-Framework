"""图4-24：涡激共振 风向–RMS–风速 极坐标散点图。

与图4-14 散点图样式对齐：角向=风向，径向=RMS，颜色=平均风速。
数据来源改为 VIV（class_1）；合并拉索后随机抽取 int(1e5) 点写入快照。
先绘制面内一张，便于预览。
"""
...
class Config:
    ...
    WEB_PAGE = "fig4_24 风向-RMS散点"
    SNAPSHOT_DIR = project_root / "results" / "chapter4_characteristics" / "figure_snapshots"
    SNAPSHOT_PATH = SNAPSHOT_DIR / "fig4_24_viv_wind_dir_rms.npz"


def _snapshot_config() -> dict:
    return {
        "figure": "fig4_24_viv_wind_dir_rms",
...
def load_pooled() -> dict:
    ...
    if not dir_parts:
        raise ValueError("无可绘制的涡激共振风向-风速-RMS 数据")
...
def push_figure(fig: plt.Figure, title: str) -> None:
    ...
    web_push(fig, page=Config.WEB_PAGE, slot=0, title=title, page_cols=1)
    print(f"[OK] 已推送到 WebUI：{Config.WEB_PAGE}")


def main() -> None:
    parser = argparse.ArgumentParser(description="图4-24 涡激共振风向-RMS-风速 极坐标散点图")
    parser.add_argument(
        "--refresh-cache",
        action="store_true",
    ...
    print("=" * 80)
    print("图4-24 涡激共振风向–RMS–风速极坐标散点图（抽样 1e5，先画面内）")
    print("=" * 80)
    ...
    print("\n[步骤2] 绘制面内极坐标散点图...")
    title = "涡激共振  面内：风向–RMS（色=平均风速）"
    fig = make_polar_scatter(
        data["wind_dirs"],
        data["rms_in"],
        data["wind_speeds"],
        title,
    )
    push_figure(fig, title)
    plt.close(fig)
    print("=" * 80)
