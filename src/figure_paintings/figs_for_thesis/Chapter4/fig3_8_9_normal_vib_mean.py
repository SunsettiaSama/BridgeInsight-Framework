...
class Config:
    ...
    N_BINS      = 100   # 主体直方图分区数（0 → 99pct），细粒度
    N_BINS_TAIL = 60    # 尾部直方图分区数（99pct → max），粗粒度

    HIST_X_PERCENTILE = 95.0        # 主体与尾部的分界分位值
    HIST_LOG_SCALE    = False        # True=对数 Y 轴；False=线性 Y 轴

    SCATTER_MAX_POINTS = 100_000    # 散点图随机下采样上限
    SCATTER_SEED = 42

    # 放大散点图轴范围（面外 RMS 为 x，面内 RMS 为 y）
    SCATTER_ZOOM_XLIM = (0.0, 0.5)
    SCATTER_ZOOM_YLIM = (0.0, 2.0)
    SCATTER_ZOOM_FIGSIZE = (5, 5)   # 正方形

    # 尾部直方图右边界：与散点图离群点边界对齐，取两轴上限的较大值
    TAIL_X_MAX = max(SCATTER_ZOOM_XLIM[1], SCATTER_ZOOM_YLIM[1])  # 2.0
...
def plot_rms_histogram(stats: dict, x_split: float) -> plt.Figure:
    ...
    ax.set_title('随机振动加速度 RMS 主体分布', fontproperties=CN_FONT, fontsize=FONT_SIZE, pad=14)
...
def plot_rms_tail_histogram(stats: dict, x_split: float) -> plt.Figure:
    ...
    # 尾部范围：(x_split, TAIL_X_MAX]，与散点图离群点边界对齐
    # 超出 TAIL_X_MAX 的数据视为极端离群点，仅在散点图中展示
    ...
    ax.set_title('随机振动加速度 RMS 尾部分布', fontproperties=CN_FONT, fontsize=FONT_SIZE, pad=14)
    ax.tick_params(axis='both', which='major', labelsize=FONT_SIZE - 2)
    _add_legend(ax)
    _apply_grid(ax)
    plt.tight_layout()
...
def plot_rms_scatter(stats: dict) -> plt.Figure:
    ...
    全量散点图：
      - 正常点（在 zoom 范围内）：小蓝点
      - 离群点（超出 zoom 范围）：大暖红点，原坐标不变，直接在全图中显示
    """
    fig, ax = plt.subplots(figsize=Config.FIG_SIZE)
    ...
    ax.set_title('随机振动 RMS 面内-面外散点图', fontproperties=CN_FONT, fontsize=FONT_SIZE, pad=14)
    ax.tick_params(axis='both', which='major', labelsize=FONT_SIZE - 2)
    _apply_grid(ax)
    plt.tight_layout()
    return fig
...
def plot_rms_scatter_zoomed(stats: dict) -> plt.Figure:
    """
    正方形局部放大散点，仅展示 zoom 范围内的局部细节。
    不参与离群点标注（离群点已在图2全量散点中显示）。
    无 title、无图例，仅保留 x/y 坐标轴标签。
    """
...
def main():
    print("=" * 80)
    print("第三章 随机振动 RMS 分布（主体 / 尾部 / 全量散点 / 放大散点）")
    print("=" * 80)
    ...
    page = "fig3_8_9 随机振动RMS"
    for slot, (fig, title) in enumerate([
        (fig_body, "RMS直方图"),
        (fig_tail, "RMS尾部直方图"),
        (fig_scatter, "面内外RMS散点"),
        (fig_zoom, "RMS散点放大"),
    ]):
