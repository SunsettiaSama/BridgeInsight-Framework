...
def _snapshot_path(nfft: int) -> Path:
    return Config.SNAPSHOT_DIR / f"fig4_22_viv_energy_cumsum_nfft{nfft}.npz"


def _snapshot_config(nfft: int) -> dict:
    return {
        "figure": "fig4_22_viv_energy_cumsum",
        "nfft": int(nfft),
...
def plot_energy_cumsum(data: dict) -> plt.Figure:
    ...
    ax.set_title(f"涡激共振前{n}阶主频累积能量分布（nfft={nfft}）", fontproperties=CN_FONT, fontsize=FONT_SIZE, pad=14)
...
def main():
    parser = argparse.ArgumentParser(description="图4-22 VIV 前50阶累积能量（支持快照）")
    parser.add_argument(
        "--refresh-cache",
        action="store_true",
    ...
    print("=" * 80)
    print("图4-22 涡激共振前50阶主频累积能量分布（nfft=128）")
    print("=" * 80)
