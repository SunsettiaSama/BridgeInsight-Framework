...
def load_psd_modes() -> dict:
    ...
    if not json_files:
        raise FileNotFoundError(f"目录下无 JSON 文件：{json_files}")

    # 每个样本的累积能量序列（按能量降序排列后归一化）
...
def _aggregate(curves: list[np.ndarray], n_modes: int) -> dict:
    """
    将长度不一的累积序列对齐到 n_modes，短于 n_modes 的用最后值填充，
    然后计算均值与标准差。
    """
...
def plot_energy_cumsum(data: dict) -> plt.Figure:
    ...
    ax.set_title('前10阶主频累积能量分布', fontproperties=CN_FONT, fontsize=FONT_SIZE, pad=14)
...
def main():
    print("=" * 80)
    print("第三章 图3.11 正常振动前10阶主频累积能量分布")
    print("=" * 80)
    ...
    web_push(fig, page="fig3_14 累积能量", slot=0, title="随机振动前10阶累积能量", page_cols=1)
    print("✓ 已推送到 WebUI")
