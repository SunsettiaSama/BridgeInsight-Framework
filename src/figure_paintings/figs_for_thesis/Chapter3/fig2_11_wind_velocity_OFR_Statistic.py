"""
图2.7: 高能量振动对应的越界风速数据统计直方图
基于极端振动样本对应的风速数据，通过step3的越界点查询，统计每个传感器的越界窗口占比
...
def calculate_sensor_statistics(metadata):
        ...
        sensor_stats: 字典，每个传感器包含：
            {
                ...
                'sensor_name': str,
                'total_windows': int,
                'velocity_out_of_range_windows': int,
                'normal_windows': int,
                'normal_ratio': float,  # 正常窗口占比
                'velocity_out_of_range_ratio': float  # 风速越界窗口占比
            }
    """
    # 按传感器统计窗口数
    sensor_window_stats = defaultdict(lambda: {'total': 0, 'velocity_out_of_range': 0})
    ...
    for item in metadata:
        ...
        # 仅统计有风速越界点的窗口数
        velocity_out_of_range_count = 0
        for window in out_of_range_windows:
            if window.get('vel_out_of_range', {}).get('count', 0) > 0:
                velocity_out_of_range_count += 1
        
        sensor_window_stats[sensor_id]['velocity_out_of_range'] += velocity_out_of_range_count
    
    # 计算每个传感器的占比
    sensor_stats = {}
    for sensor_id, stats in sensor_window_stats.items():
        total = stats['total']
        velocity_out_of_range = stats['velocity_out_of_range']
        normal = total - velocity_out_of_range
        
        sensor_stats[sensor_id] = {
            'sensor_id': sensor_id,
            'sensor_name': WIND_SENSOR_NAMES.get(sensor_id, '未知'),
            'total_windows': total,
            'velocity_out_of_range_windows': velocity_out_of_range,
            'normal_windows': normal,
            'normal_ratio': normal / total if total > 0 else 0.0,
            'velocity_out_of_range_ratio': velocity_out_of_range / total if total > 0 else 0.0
        }
...
def plot_sensor_statistics_histogram(sensor_stats):
    ...
    bar_color = blue_cmap(1)  # 选择中间偏深的蓝色
...
def print_statistics_summary(sensor_stats):
    ...
    """
    print(f"\n{'='*80}")
    print(f"{'传感器风速越界窗口占比统计摘要':^80}")
    print(f"{'='*80}")
    
    # 创建表格头
    print(f"{'传感器ID':<12} {'传感器名称':<15} {'总窗口数':>10} {'风速越界窗口':>12} {'正常窗口':>10} {'正常占比':>12}")
    print(f"{'-'*80}")
    
    # 统计全局数据
    total_all_windows = 0
    total_velocity_out_of_range_windows = 0
    ...
    for sensor_id in sorted(sensor_stats.keys()):
        ...
        print(
            ...
            f"{stats['total_windows']:>10} {stats['velocity_out_of_range_windows']:>12} "
            f"{stats['normal_windows']:>10} {stats['normal_ratio']*100:>11.2f}%"
        )
        
        total_all_windows += stats['total_windows']
        total_velocity_out_of_range_windows += stats['velocity_out_of_range_windows']
    ...
    if len(sensor_stats) > 0:
        ...
        global_normal_ratio = (total_all_windows - total_velocity_out_of_range_windows) / total_all_windows if total_all_windows > 0 else 0.0
        
        print(f"\n全局统计:")
        print(f"  总窗口数: {total_all_windows}")
        print(f"  风速越界窗口数: {total_velocity_out_of_range_windows}")
        print(f"  正常窗口数: {total_all_windows - total_velocity_out_of_range_windows}")
...
def main():
    ...
    """
    print("="*80)
    print(" "*15 + "图2.7: 高能量振动对应的越界风速数据统计直方图")
    print("="*80)
    ...
    print(f"✓ 数据来源: 极端振动对应的风速数据（通过 step3 越界点查询）")
