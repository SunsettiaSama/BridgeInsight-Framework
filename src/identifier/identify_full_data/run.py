...
使用方式
--------
    python -m src.identifier.identify_full_data.run \
        ...
        --config   config/identifier/identify_full_data/default.yaml
"""
...
from src.config.identifier.identify_full_data.config import (
    IdentifyFullDataConfig,
    load_config,
)
from src.identifier.identify_full_data._modal import (
    compute_psd_top_modes,
    compute_spectral_features,
)
from src.identifier.identify_full_data._signal import compute_time_stats
from src.identifier.identify_full_data._coupling import compute_cross_coupling
from src.identifier.identify_full_data._wind import (
    build_wind_lookup,
...
from src.identifier.identify_full_data._splitter import save_class_results
...
# ---------------------------------------------------------------------------
_worker_cfg: Optional[IdentifyFullDataConfig] = None


def _init_worker(cfg: IdentifyFullDataConfig) -> None:
    global _worker_cfg
    _worker_cfg = cfg
...
def run(
    ...
    output_dir: str,
    cfg: Optional[IdentifyFullDataConfig] = None,
    ...
    for idx_str, pred_cls in predictions.items():
        ...
        # 折减风速（依赖 psd_inplane 的第一主频 + wind_stats 的均值风速）
        if cfg.enable_reduced_velocity and cfg.cable_diameter:
            vr_list = []
            ...
            for ws in wind_stats:
                ...
                if f1 and U is not None:
                    vr = compute_reduced_velocity(U, f1, cfg.cable_diameter)
                    vr_list.append({
    ...
    saved = save_class_results(
        ...
        source_result_name=Path(result_path).name,
