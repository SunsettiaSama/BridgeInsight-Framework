...
class DataManager():
    ...
    def __init__(self):
        ...
        self.sensor_ids = {
           "wind_sensors": {"跨中桥面上游": "ST-UAN-G04-001-01", 
                            "跨中桥面下游": "ST-UAN-G04-002-01", 
                            "北塔下游江侧桥塔与风障开槽处": "ST-UAN-G02-002-01", 
                            "北塔下游江侧风障末尾处": "ST-UAN-G02-002-02", 
                            "北索塔塔顶": "ST-UAN-T01-003-01", 
                            "南索塔塔顶": "ST-UAN-T02-003-01",}, 
            "vibration_sensors": [], 
        }
        return 
    
    def filter_files_by_sensor_id(self, file_paths: list, sensor_id: str) -> list:
        """
        1. 检查 sensor_id 是否合法（中文名称或传感器ID）
        2. 筛选包含对应传感器ID的文件路径
        """
        # 构建所有传感器ID的集合和中文名称到ID的映射
        all_sensors = set()
        name_to_id = {}
        
        for sensor_type in ["wind_sensors", "vibration_sensors"]:
            sensors = self.sensor_ids[sensor_type]
            if isinstance(sensors, dict):  # 处理字典结构（中文名称 -> ID）
                for name, sid in sensors.items():
                    all_sensors.add(sid)
                    name_to_id[name] = sid
            elif isinstance(sensors, list):  # 处理列表结构（仅 ID）
                for sid in sensors:
                    all_sensors.add(sid)
            else:
                raise ValueError(f"Invalid sensor structure for {sensor_type}")

        # 检查输入是否合法
        if sensor_id in name_to_id:
            # 输入是中文名称，转换为传感器ID
            sensor_id = name_to_id[sensor_id]
        elif sensor_id not in all_sensors:
            raise ValueError(f"Sensor ID or name '{sensor_id}' not found in any sensor list.")

        # 筛选文件路径：文件名包含 sensor_id（不区分大小写）
        filtered_files = []
        for path in file_paths:
            filename = os.path.basename(path).lower()
            if sensor_id.lower() in filename:
                filtered_files.append(path)

        return filtered_files
    
    def clean_data(self, data: list, min_num: float = -40, max_num: float = 40):
        """
        将异常值，超过阈值的部分进行清洗，清洗完成的部分视为np.nan
        风速参考值：0~70m/s（海上极端台风天气】
        """
        ...
        for i, item in enumerate(data):
            if max_num is not None:
                if item > max_num:
                    data[i] = np.nan
            if min_num is not None and item < min_num:
                if item < min_num:
                    data[i] = np.nan

        return data
    def clean_nan_and_align(self, *args):
        """
        清洗输入列表中的 NaN 值，并对齐多个列表。
        
        参数:
            *args: 可变数量的列表，每个列表应为相同长度。
        
        返回:
            如果传入一个列表: 返回清洗后的列表。
            如果传入多个列表: 返回清洗后的多个列表，保持对齐。
        """
        # 单个列表：直接清洗 NaN
        if len(args) == 1:
            arr = np.array(args[0])
        ...
        if not all(len(lst) == first_len for lst in args):
    ...
    def get_wind_data_from_root(self, wind_dir, mode = "mean", sensor_id = None, bin_nums = 10):
        ...
        if sensor_id is not None:
            file_paths_lis = self.filter_files_by_sensor_id(file_paths_lis, sensor_id)

        wind_velocities, wind_directions, wind_angles = [], [], []
        
        for wind_path in file_paths_lis:
            wind_velocity, wind_direction, wind_angle = self.unpacker.Wind_Data_Unpack(wind_path)
            # 以np.nan作为掩码
            wind_velocity = self.clean_data(wind_velocity, 1, 70)
            wind_direction = self.clean_data(wind_direction, 0, 360)
            wind_angle = self.clean_data(wind_angle, 0, 180)
            # 统一清洗
            wind_velocity, wind_direction, wind_angle = self.clean_nan_and_align(wind_velocity, wind_direction, wind_angle)

            if mode == "mean":
                wind_velocities.append(np.mean(wind_velocity))
                wind_directions.append(np.mean(wind_direction))
                wind_angles.append(np.mean(wind_angle))
            if mode == "all":
                wind_velocities.extend(wind_velocity)
            ...
            if mode == "turbulence":
                ...
                for i in range(bin_nums):
                    ...
                    wind_velocity_interval = np.mean(wind_velocity[i * interval_length: (i + 1) * interval_length])
                    wind_velocity_interval_mean = np.mean(wind_velocity_interval)
                    wind_velocity_interval_rms = np.sqrt(np.mean((wind_velocity_interval - wind_velocity_interval_rms) ** 2))
                    Wind_TI = wind_velocity_interval_rms / wind_velocity_interval_mean
